...
import com.y.wirelesstemperaturemeasurement.data.parse.EventLevelAndMsg
...
object MyMQTT : MqttCallback {
    ...
    fun init() {
        ...
        mqttConnectionOptions.setAutomaticReconnectDelay(2, 300)
        try {
            MQTT.connect(mqttConnectionOptions)
        } catch (e: Exception) {
            Log.e(TAG, "init: MQTT连接失败", e)
        }
    }
